...
data: 
	...
	cd ./tool && python gen_data.py --input_nhwc=$(INPUT_NHWC) --conv_weight_nhwc=$(CONV_WEIGHT_NHWC) --fc_weight_trans=$(FC_WEIGHT_TRANS)
...
clear_data: clean_data
	@$(call remove_dir,./tool/__pycache__)

clean: clean_hw clean_sw clean_data

clear: clear_hw clear_sw clear_data