...
// 向量长度定义 (与硬件一致)
#define VLMAX 8  // 512-bit / 64-bit = 8 elements

// Softmax 查找表大小 (需与 gen_data.py 一致)
#define LUT_SIZE 256

/**
 * @brief 权重预处理 (Flatten) - 向量版本
...
/**
 * @brief 矩阵元素级加法 (通常用于 Bias Add) - 向量版本
 * C = A + B
 * @param len 元素总数
 */
...
/**
 * @brief MaxPool 2x2 (NHWC 格式) - 向量版本
 * Input:  [H, W, C]
 * Output: [H/2, W/2, C]
 */
...
/**
 * @brief Transpose NHWC to NCHW - 向量版本
 * Input:  [H, W, C]
 * Output: [C, H, W]
 */
